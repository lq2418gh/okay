...
<body>
	<div id="context">
		<div class="container">
			<form:form id="direct" name="direct" action="${ctx }/direct/save.do" commandName="direct">
				<div class="row">
					<div class="span12">
								...
								<div class="panel-heading">
									<h4 class="panel-title">
										<a id="example" data-toggle="collapse" data-parent="#accordion" href="#collapseOne" style="display: block;"> 
											<span class="label label-primary">物资库存管理>>直达现场>>编辑</span>
										</a>
									</h4>
								</div>
								<div id="collapseOne" class="panel-collapse collapse in">
									<div class="panel-body">
										...
										<table id="queryTable" class="table table-condensed">
											<tr>
												<th ><span class="glyphicon glyphicon-asterisk"style="color: white"></span>直达现场单编号</th>
												<td ><input type="text" name="direct_no" id="direct_no"  value="${direct.direct_no }" class="form-control input-sm" readonly="readonly" /></td>
												
												<th ><span class="glyphicon glyphicon-asterisk" style="color: red"></span>入库时间</th>
												<td>
													<input type="text" name="entry_date" readonly="readonly" id="entry_date" class="form-control input-sm form_datetime"  value="<fmt:formatDate value="${direct.entry_date }" pattern="yyyy-MM-dd"/>"/> 
												</td>
												<th ><span class="glyphicon glyphicon-asterisk"style="color: white"></span>状态</th>
												<td ><input type="text" name="state" id="state"  value="${direct.state }" class="form-control input-sm" readonly="readonly" value="${direct.state }"/></td>
												
											</tr>
											<tr>
												<th><span class="glyphicon glyphicon-asterisk" style="color: red"></span>采购单编号</th>
												<td >
														<input type="text" name="order_no" value="${direct.order_no }" class="form-control input-sm " style="width:70%" readonly="readonly" /> 
													...
													<input type="hidden" name="order_frame" value="${direct.order_frame }"/>
												</td>
												<th><span class="glyphicon glyphicon-asterisk"style="color: white"></span>合同/协议 </th>
												<td>
												<c:if test="${empty direct.frame_no && empty direct.contract_no }">
												<input type="text" name="contract_no" id="contract_no" class="form-control input-sm" readonly="readonly" />
												</c:if>
												<c:if test="${not empty direct.frame_no }">
												<input type="text" name="frame_no" id="frame_no"  value="${direct.frame_no }" class="form-control input-sm" readonly="readonly" />
												</c:if>
												<c:if test="${not empty direct.contract_no }">
												<input type="text" name="contract_no" id="contract_no"  value="${direct.contract_no }" class="form-control input-sm" readonly="readonly" />
												</c:if>
												</td>
												<th><span class="glyphicon glyphicon-asterisk"style="color: white"></span>供应商名称 </th>
												<td><input type="text" name="supplier_name" id="supplier_name"  value="${direct.supplier_name }" class="form-control input-sm" readonly="readonly" /></td>
											</tr>
											<tr>
												...
												<td>
													<html:select id="department" name="department" classes="form-control input-sm">
														<html:businessOptions function="company" condition="" checkValue="${direct.department }"></html:businessOptions>
													</html:select>
												</td>
												<th><span class="glyphicon glyphicon-asterisk"style="color: white"></span>购置经办人</th>
												<td><input type="text" name="user_name" id="user_name" value="${direct.user_name }" class="form-control input-sm" readonly="readonly" />
												    <input type="hidden" name="user_id" value="${direct.user_id }"/>
												</td>
												<th></th>
												<td></td>
											</tr>
											<tr>
												...
												<security:authorize ifAnyGranted="ROLE_DIRECT_ADD">
													...
													<a class="btn btn-primary btn-xs" href="${ctx }/direct/view.do?id=${direct.id}">返回</a>
													</c:if>
												</td>
											</tr>
										</table>
...
function handleOrder(order) {
	...
	object.find("input[name='supplier_name']").val(order.supplier_name);
	//显示协议合同号
	if(isEmpty(order.contract_no)){
	object.find("input[name='contract_no']").val(order.frame_no);
	}else{
	object.find("input[name='contract_no']").val(order.contract_no);
	}
}
...
function save(oTb) {
	...
	changeDetails("mxtable", "directDetailList",2);
